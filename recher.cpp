    ...
    model(new QStringListModel(this))
{
    ...
    QObject::connect(A.getserial(), &QSerialPort::readyRead, this, &recher::update_label);
    ...
    connect(ui->push_recherche, &QPushButton::clicked, this, &recher::onSearchClicked);
    connect(manager, &QNetworkAccessManager::finished, this, &recher::onApiResult, Qt::UniqueConnection);
    connect(ui->Rc_Line_Recherche, &QLineEdit::textChanged, this, &recher::on_Rc_Line_Recherche_textChanged);
    ...
    connect(ui->tableView_Recherche, &QTableView::doubleClicked, this, &recher::on_tableView_Recherche_doubleClicked);
...
void  recher::populateComboBoxes()
{
...
void recher::on_Rc_push_Supprimer_clicked()
{
    bool ok;
...
void recher::on_Rc_push_Modifier_clicked()
{
    bool ok;
    int ID = ui->RC_combo_ID->currentText().toInt(&ok);
    if (!ok) {
...
void recher::on_Rc_push_Ajouter_clicked()
{
    bool ok;
...
void recher::on_RC_combo_ID_currentIndexChanged(int index)
{
    if (index == -1) return;
...
void recher::applyTableTranslations()
{
...
void recher::on_Rc_Button_Tri_Nom_clicked()
{
    ui->Rc_TableView_Res->setModel(R.Afficher_Tri_NOM());
    applyTableTranslations();
}

void recher::on_Rc_Button_Tri_Type_clicked()
{
    ui->Rc_TableView_Res->setModel(R.Afficher_Tri_TYPE());
    applyTableTranslations();
}

void recher::on_Rc_Button_Tri_Statut_clicked()
{
    ui->Rc_TableView_Res->setModel(R.Afficher_Tri_STATUT());
    applyTableTranslations();
}

void recher::on_Rc_Line_Recherche_textChanged(const QString &arg1)
{
...
void recher::on_Rc_Button_ExportPDF_clicked()
{
...
void recher::on_Rc_Button_Stat_clicked()
{
    stati *statWindow = new stati(this);
...
void recher::onApiResult(QNetworkReply *reply) {
...
void recher::on_line_Recherche_textChanged(const QString &searchText) {
...
void recher::on_tableView_Recherche_doubleClicked(const QModelIndex &index)
{
    int row = index.row();
...
bool recher::loadTranslations(const QString &filePath)
{
    QFile file(filePath);
...
void recher::on_push_Translation_clicked()
{
...
void recher::on_push_recherche_clicked()
{

}
bool recher::connectArduino()
{
    ...
    if (arduino->open(QIODevice::ReadWrite)) {
        connect(arduino, &QSerialPort::readyRead, this, &recher::readSerial);
        return true;
    } else {
        return false;
    }
}
void recher::readSerial()
{
...
void recher::on_pushButton_envoyer_clicked()
{
    QSqlQuery query;
...
void recher::on_pushButton1_envoyer_clicked()
{
